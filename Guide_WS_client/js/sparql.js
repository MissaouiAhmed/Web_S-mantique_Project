...
/* Ayoub function return Monuments in reel time*/
 function getReelMonuments (ville){
		 var tableContent ='<thead><tr> <br/> <td>x</td></tr></thead> <tbody>';
		 //var url = "http://localhost:3030/data/query";		 
 var url = "http://localhost:8080/sparql/template?profile=st%3Aldp&query=";
		 //var query = "\PREFIX tourisme: <http://www.polytech.semantique/tourisme%23> "+
		 var query = "PREFIX dbo:<http://dbpedia.org/ontology/>PREFIX : <http://dbpedia.org/resource/>PREFIX foaf:<http://xmlns.com/foaf/0.1/>PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema%23>PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns%23>PREFIX dbpprop: <http://dbpedia.org/property/>PREFIX grs:<http://www.georss.org/georss/>SELECT ?description ?imageLink ?website WHERE { service <http://dbpedia.org/sparql> { {?x rdf:type dbo:Museum;dbo:abstract ?description;dbo:thumbnail ?imageLink;dbpprop:pushpinMap '"+ville+"'@en;dbpprop:website ?website;grs:point ?localisation.FILTER(langMatches(lang(?description), 'FR'))} UNION {?x rdf:type dbo:Museum;dbo:abstract ?description;dbo:thumbnail ?imageLink;dbo:location '"+ville+"'@en;dbpprop:website ?website;grs:point ?localisation.FILTER(langMatches(lang(?description), 'FR'))}}}";
